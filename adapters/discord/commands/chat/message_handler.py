...
class MessageHandler:
    def __init__(self, cog: commands.Cog):
        ...
        self._multiagent_personas = {}
        self._persona_other_personas = {}
        self._active_conversations = {}
        self._selected_persona_for_response = None
...
JSON only:"""
    ...
    async def check_and_handle_message(self, message: discord.Message) -> bool:
        ...
        # 5. Persona Selection + Behavior Engine Decision
        # First select which persona should respond, then ask BehaviorEngine if they want to
        selected_persona = None

        if (
            not should_respond
            and hasattr(self.cog, "persona_router")
            and self.cog.persona_router
        ):
            try:
                # Select persona based on message context
                selected_persona = self.cog.persona_router.select_persona(
                ...
                if selected_persona:
                    logger.debug(
                        f"PersonaRouter selected: {selected_persona.character.display_name}"
                    )
            except Exception as e:
                logger.warning(f"PersonaRouter selection failed: {e}")

        # Now ask BehaviorEngine if this specific persona wants to respond
        if (
            not should_respond
            ...
            try:
                # Pass the selected persona to BehaviorEngine
                decision = await self.cog.behavior_engine.handle_message(
                    message, persona=selected_persona
                )
                ...
                if decision:
                    ...
                    if decision.get("should_respond"):
                        ...
                        # Store selected persona for response generation
                        if selected_persona:
                            self._selected_persona_for_response = selected_persona
                        logger.info(
                            f"✨ Behavior Engine: RESPOND - Persona: {selected_persona.character.display_name if selected_persona else 'default'}, Reason: {decision.get('reason')}, Style: {suggested_style}"
                        )
                    else:
                        logger.debug(
        ...
        if should_respond:
            ...
            # Determine which persona to use for response
            # Priority: 1) Selected by BehaviorEngine, 2) Mentioned in message, 3) Current default
            response_persona = None
            if self._selected_persona_for_response:
                response_persona = self._selected_persona_for_response
                self._selected_persona_for_response = None  # Clear after use
            elif hasattr(self.cog, "current_persona") and self.cog.current_persona:
                response_persona = self.cog.current_persona
            ...
            if response_persona:
                p_id = getattr(response_persona, "character_id", None)
                if not p_id:
                    p_id = getattr(response_persona, "persona_id", None)
                if not p_id:
                    p_id = getattr(response_persona, "id", "unknown")
            ...
            async def respond_callback(
                ...
                async with message.channel.typing():
                    # Temporarily set the persona for this response
                    original_persona = self.cog.current_persona
                    if response_persona:
                        self.cog.current_persona = response_persona
                        if self.cog.behavior_engine:
                            self.cog.behavior_engine.set_persona(response_persona)
                    ...
                    try:
                        ...
                        if original_persona:
                            self.cog.current_persona = original_persona
                            if self.cog.behavior_engine:
                                self.cog.behavior_engine.set_persona(original_persona)
