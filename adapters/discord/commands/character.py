...
class CharacterCommandsCog(commands.Cog):
    ...
    async def set_character(
        ...
        try:
            # Get ChatCog to access PersonaRouter
            chat_cog = self.bot.get_cog("ChatCog")
            if not chat_cog or not chat_cog.persona_router:
                await interaction.followup.send(
                    "❌ PersonaRouter not initialized. Check that the chat system is enabled.",
                    ephemeral=True,
                )
                return
            ...
            # Compile persona using PersonaRouter's system
            compiled_persona = chat_cog.persona_router.persona_system.compile_persona(
            ...
            # Update ChatCog's current persona (primary way to set active persona)
            chat_cog = self.bot.get_cog("ChatCog")
            if chat_cog:
                chat_cog.current_persona = compiled_persona
                chat_cog.compiled_persona = compiled_persona
                chat_cog.system_prompt = compiled_persona.system_prompt

                # Update BehaviorEngine (replaces decision_engine, ambient_mode, curiosity_system)
                if chat_cog.behavior_engine:
                    chat_cog.behavior_engine.set_persona(compiled_persona)
                    logger.info(
                        f"Updated BehaviorEngine with new persona: {compiled_persona.persona_id}"
                    )
