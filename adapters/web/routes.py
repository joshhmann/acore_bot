...
def create_router(
    ...
    async def chat(request: ChatRequest) -> ChatResponse:
        ...
        """
        try:
            runtime_response = await runtime.handle_event(
            ...
            return ChatResponse(
                response=runtime_response.text,
                persona_id=runtime_response.persona_id,
                persona_name=runtime_response.persona_id,
    ...
    async def runtime_event(
        ...
        """Send event to runtime and return envelope."""
        _require_auth(http_request)
        client_scope = extract_request_client_scope(http_request)
        claimed_user_id = extract_request_user_id(http_request)
