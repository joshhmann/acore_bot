...
from core.types import AcoreChannel, AcoreUser
...
class ContextResult:
    ...
    participants: List[str]  # User IDs in conversation
...
class ContextRouter:
    ...
    def __init__(self, history_manager, summarizer=None):
        ...
        self.channel_summaries: Dict[str, str] = {}

        logger.info("Context router initialized")

    def detect_channel_type(self, channel: AcoreChannel) -> str:
        """
        Detect channel type.

        Args:
            channel: AcoreChannel instance
    ...
    async def get_context(
        self, channel: AcoreChannel, user: AcoreUser, message_content: str
    ) -> ContextResult:
        """
        ...
        Args:
            channel: AcoreChannel instance
            user: AcoreUser instance (message author)
        ...
        should_summarize = await self._should_summarize(
            channel_id, len(full_history), strategy
        )
        ...
        if should_summarize and self.summarizer:
            summary = await self._get_or_create_summary(
                channel_id, full_history, strategy
            )

        # For threads, try to inherit parent context
        if strategy.inherit_parent and channel.type == "thread":
    ...
    async def _should_summarize(
        self, channel_id: str, message_count: int, strategy: ContextStrategy
    ) -> bool:
        """
        ...
        if message_count >= strategy.max_messages:
            logger.info(
                f"Channel {channel_id}: {message_count} messages, triggering summarization"
            )
            return True
        ...
        if last_activity:
            ...
            if idle_minutes >= strategy.idle_timeout_minutes:
                logger.info(
                    f"Channel {channel_id}: {idle_minutes:.1f}min idle, triggering summarization"
                )
                return True
    ...
    async def _get_or_create_summary(
        self, channel_id: str, history: List[Dict[str, str]], strategy: ContextStrategy
    ) -> Optional[str]:
        """
    ...
    async def _get_parent_summary(self, channel: AcoreChannel) -> Optional[str]:
        """
        Get summary from parent channel for thread context.

        Args:
            channel: AcoreChannel instance (thread)
    ...
    def clear_voice_context(self, channel_id: str):
        """
